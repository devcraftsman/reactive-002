...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] =  {
      Observable(obs => {
        val txtReact = Reaction {
          case ValueChanged(f) => obs.onNext(f.text)
          case _ =>
        }
        field.subscribe(txtReact)
        Subscription {
          field.unsubscribe(txtReact)
          obs.onCompleted()
        }
      })
    }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = Observable{
      observer => {
        val buttonReact = Reaction{
          case ButtonClicked(b) => observer.onNext(b)
          case _ =>
        }
        button.subscribe(buttonReact)
        Subscription {
          button.unsubscribe(buttonReact)
